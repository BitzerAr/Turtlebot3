...
//Servomotores
diagnostic_msgs::DiagnosticStatus servo_state;
diagnostic_msgs::DiagnosticStatus electro_state;
...
//ServoMotor
void setServoDiagnosis(uint8_t level, std::string message){
	setDiagnosisMsg(&servo_state, level, "Servomotor", message, "Servo");
}
//Electroiman
void setElectroDiagnosis(uint8_t level, std::string message){
	setDiagnosisMsg(&electro_state, level, "Electroiman", message, "Electroiman");
}
...
void sensorStateMsgCallback(const turtlebot3_msgs::SensorState::ConstPtr &msg)
{
  ...
  if (msg->torque == true)
	...
	//Servo
	if (msg->angle == true)
		setServoDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "Angle ON");
	else	
		setServoDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "Angle OFF");
	//Electroiman
	if (msg->state == true)
		setElectroDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "Electro ON");
	else	
		setElectroDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "Electro OFF");
}
...
void msgPub()
{
  ...
  tb3_diagnostics.status.push_back(button_state);
	tb3_diagnostics.status.push_back(servo_state);
	tb3_diagnostics.status.push_back(electro_state);
